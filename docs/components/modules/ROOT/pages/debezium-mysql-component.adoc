...
The Debezium MySQL Connector component supports 78 options, which are listed below.
...
| *columnMaskHashWithSalt* (mysql) | A comma-separated list of regular expressions matching fully-qualified names of columns that should be masked by hashing the input. Using the specified hash algorithms and salt. |  | String
| *columnMaskWithdChars* (mysql) | A comma-separated list of regular expressions matching fully-qualified names of columns that should be masked with configured amount of asterisk ('') characters. |  | String
| *columnTruncateTodChars* (mysql) | A comma-separated list of regular expressions matching fully-qualified names of columns that should be truncated to the configured amount of characters. |  | String
