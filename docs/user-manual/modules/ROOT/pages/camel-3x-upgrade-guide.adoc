...
The `camel-milo` server component properties `hostname` and `strictEndpointUrlsEnabled` are no longer supported.
Property `hostName` and `serverName` are replaced by `path`.
To successfully use certificates for secured communication, JCE Jurisdiction Policy File Default
has to be *Unlimited* (which is by default since jdk 9+).
