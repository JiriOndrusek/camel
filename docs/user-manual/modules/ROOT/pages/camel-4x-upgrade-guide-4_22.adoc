...
* `HYBRID_QUERY` — Keyword + vector hybrid search. The exchange body is the query text (`String`). Supports `CamelWeaviateQueryTopK` (default 10), `CamelWeaviateHybridAlpha` (0.0 = pure BM25, 1.0 = pure vector), and `CamelWeaviateFields` headers. Returns a `QueryResponse<Map<String, Object>>`.
...
* `AGGREGATE` — Returns aggregate statistics for a collection. No body required for total count only. Optionally pass a `List<PropertyAggregation>` (from `io.weaviate.client6.v1.api.collections.aggregate`) as the body to request per-property metrics (e.g., text top-occurrences, numeric min/max/mean/sum). The optional `CamelWeaviateAggregateObjectLimit` header (`Integer`) limits how many objects are considered. Returns an `AggregateResponse` with `totalCount()` and typed per-property accessors.
