...
elytron:ws://hostname[:port][/resourceUri][?options]
elytron:wss://hostname[:port][/resourceUri][?options]
----
...
The Elytron component supports 9 options, which are listed below.
...
| *securityDomainBuilder* (elytron) | TODO |  | Builder
...
|===
| Name | Description | Default | Type
| *allowedRoles* (allowedRoles) | TODO Comma separated lost of allowed roles. |  | List
...
| *camel.component.elytron.security-domain-builder* | TODO. The option is a org.wildfly.security.auth.server.SecurityDomain.Builder type. |  | String
