...
(MechanusmName should be selected with regard to default securityRealm. For example: to use bearer_token security, mechanism name has to be "BEARER_TOKEN"
...
The Elytron component supports 10 options, which are listed below.
...
| *securityDomainBuilder* (elytron) | Definition of Builder, which will be used for creation of security domain. |  | Builder
...
|===
| Name | Description | Default | Type
| *allowedRoles* (allowedRoles) | Comma separated list of allowed roles. |  | List
