...
public class WeaviateVectorDbProducer extends DefaultProducer {
    ...
    private void aggregate(Exchange exchange) throws Exception {
        ...
        List<PropertyAggregation> metrics = body instanceof List ? (List<PropertyAggregation>) body : null;
        Integer objectLimit = in.getHeader(WeaviateVectorDbHeaders.AGGREGATE_OBJECT_LIMIT, Integer.class);

        AggregateResponse result = (AggregateResponse) collection.aggregate.overAll(a -> {
            a.includeTotalCount(true);
            if (metrics != null && !metrics.isEmpty()) {
                a.metrics(metrics.toArray(new PropertyAggregation[0]));
            }
            if (objectLimit != null) {
                a.objectLimit(objectLimit);
            }
            return a;
        });
