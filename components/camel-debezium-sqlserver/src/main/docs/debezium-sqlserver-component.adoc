...
The Debezium SQL Server Connector component supports 58 options, which are listed below.
...
| *columnMaskHashWithSalt* (sqlserver) | A comma-separated list of regular expressions matching fully-qualified names of columns that should be masked by hashing the input. Using the specified hash algorithms and salt. |  | String
| *columnMaskWithdChars* (sqlserver) | A comma-separated list of regular expressions matching fully-qualified names of columns that should be masked with configured amount of asterisk ('') characters. |  | String
| *columnTruncateTodChars* (sqlserver) | A comma-separated list of regular expressions matching fully-qualified names of columns that should be truncated to the configured amount of characters. |  | String
